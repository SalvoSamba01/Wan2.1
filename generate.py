...
def _parse_args():
    ...
    parser.add_argument("--frame_num", type=int, default=None, help="How many frames to sample from an image or video. The number should be 4n+1")
    ...
    parser.add_argument("--save_file", type=str, default=None, help="The file to save the generated image or video to. (Ignored when using CSV mode)")
    parser.add_argument("--prompt", type=str, default=None, help="The prompt to generate the image or video from.")
    parser.add_argument("--csv_file", type=str, default=None, help="Path to CSV file containing prompts (one prompt per riga).")
...
def generate(args):
    ...
    # Per task t2v o t2i, prepariamo la lista dei prompt
    if "t2v" in args.task or "t2i" in args.task:
        if args.csv_file is not None:
            logging.info(f"Reading prompts from CSV file: {args.csv_file}")
            # Legge il file riga per riga
            with open(args.csv_file, "r", encoding="utf-8") as f:
                prompt_list = [line.strip() for line in f if line.strip()]
            if not prompt_list:
                raise ValueError("Il file CSV non contiene alcun prompt valido.")
        else:
            if args.prompt is None:
                args.prompt = EXAMPLE_PROMPT[args.task]["prompt"]
            prompt_list = [args.prompt]

        logging.info(f"Input prompt(s): {prompt_list}")
        ...
        for idx, prompt in enumerate(prompt_list):
            ...
            logging.info(f"Processing prompt {idx}: {current_prompt}")
            if args.use_prompt_extend:
                logging.info("Extending prompt ...")
                if rank == 0:
                    prompt_output = prompt_expander(current_prompt, tar_lang=args.prompt_extend_target_lang, seed=args.base_seed)
                ...
                if dist.is_initialized():
                    dist.broadcast_object_list(current_prompt, src=0)
            ...
            if rank == 0:
                ...
                save_file = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{args.ulysses_size}_{args.ring_size}_{safe_prompt}_{formatted_time}_{idx}" + suffix
                if "t2i" in args.task:
                ...
                logging.info(f"Finished processing prompt {idx}.")
    else:
        # Branch per i2v (Image-to-Video), il CSV non viene considerato in questo ramo
        if args.prompt is None:
            args.prompt = EXAMPLE_PROMPT[args.task]["prompt"]
        ...
        if args.use_prompt_extend:
            ...
            if rank == 0:
                ...
                args.prompt = [None]
            if dist.is_initialized():
                dist.broadcast_object_list(args.prompt, src=0)
